...
export function getFinalAmount(amount, conversionFee) {
    [amount, conversionFee] = Array.from(arguments).map(x => new Decimal(x));
    return amount.mul(MAX_FEE.sub(conversionFee)).div(MAX_FEE);
}
